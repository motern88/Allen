...
1. 将消息分发给对应的 Agent（会调用`agent.receive_message`方法来处理消息）
2. 记录该成功分发的消息到任务会话池中（追加到`task_state.shared_conversation_pool`中）
...
在前端界面中，我们需要以合理的方式展示一条消息的重要属性。面对历史消息记录：
...
- 消息类型：
  ...
  - 需要回复，发送者并不等待该消息的回复。
    ...
    需要回复但不紧急的消息应当以某种程度的区分来提示人类操作员，并且应当支持便捷的快速回复。

  - 需要立即回复，发送者正在等待该回复。

    需要紧急回复的消息应当以某种程度的区分来提示人类操作员，并且应当支持便捷的快速回复。
...
- 消息接收者：
  ...
  我们需要看到该消息发送的对象，同时如果该消息的接收者包含自身，则需要一种方式来标明以提醒。
